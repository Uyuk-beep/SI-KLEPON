...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add' || $action === 'edit') {
        $pengumumanId = $_POST['pengumuman_id'] ?? 0;
        ...
        if (mb_strlen($judul) > 200) {
            $error = 'Judul pengumuman tidak boleh melebihi 200 karakter.';
        } elseif (mb_strlen($deskripsi) > 2000) {
            $error = 'Isi pengumuman tidak boleh melebihi 2000 karakter.';
        } elseif (empty($judul) || empty($deskripsi)) {
            $error = 'Judul dan isi pengumuman harus diisi.';
        } else {
            
            // Proses Upload Foto (Menggunakan 'foto')
            if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
                $uploadResult = uploadFile($_FILES['foto'], 'pengumuman');
                
                if ($uploadResult) {
                    // Jika upload file BARU berhasil, baru hapus file LAMA (jika sedang mode edit)
                    if ($action === 'edit' && $pengumumanId > 0) {
                        $oldPengumuman = Database::query("SELECT foto FROM pengumuman WHERE pengumuman_id = ?", [$pengumumanId])->fetch();
                        $oldFotoPath = $oldPengumuman['foto'] ?? '';
            ...
            if (empty($error)) {
                if ($action === 'add') {
                    $sql = "INSERT INTO pengumuman (judul, deskripsi, foto) 
                              VALUES (?, ?, ?)";
                    $params = [$judul, $deskripsi, $newFoto];
                    
                    if(Database::query($sql, $params)) {
                         Session::set('success', 'Pengumuman berhasil ditambahkan');
                    } else {
                    ...
                    if ($fotoUpdated) {
                        $sql = "UPDATE pengumuman SET judul = ?, deskripsi = ?, foto = ? WHERE pengumuman_id = ?";
                        $params = [$judul, $deskripsi, $newFoto, $pengumumanId];
                    } else {
                        $sql = "UPDATE pengumuman SET judul = ?, deskripsi = ? WHERE pengumuman_id = ?";
                        $params = [$judul, $deskripsi, $pengumumanId];
                    }
                    
                    if(Database::query($sql, $params)) {
                        Session::set('success', 'Pengumuman berhasil diperbarui');
                    } else {
        ...
        if(Database::query("DELETE FROM pengumuman WHERE pengumuman_id = ?", [$pengumumanId])->rowCount() > 0) {
            // 3. JIKA record berhasil dihapus, baru hapus file fisiknya
            $oldFotoPath = $oldPengumuman['foto'] ?? ''; 
            if (!empty($oldFotoPath) && defined('UPLOAD_DIR') && file_exists(UPLOAD_DIR . $oldFotoPath)) {
                @unlink(UPLOAD_DIR . $oldFotoPath);
            }
            Session::set('success', 'Pengumuman berhasil dihapus.');
        } else {
            Session::set('error', 'Gagal menghapus pengumuman.');
        }
    }
...
// Ambil semua pengumuman (KUERI UTAMA UNTUK MENAMPILKAN DATA)
...
if ($use_dummy_data && count($pengumuman) < 10) {
    for ($i = 1; $i <= 100; $i++) {
        $pengumuman[] = [
            'pengumuman_id' => 8000 + $i,
            'judul' => 'Pengumuman Dummy ' . $i,
            'deskripsi' => 'Ini adalah isi dari pengumuman dummy nomor ' . $i . ' untuk tujuan pengujian dan pengembangan. Harap abaikan konten ini.',
...
<head>
    ...
    <style>
        /* --- CSS UNTUK TRUNCATE/POTONG TEKS PADA TABEL --- */
        /* Digunakan untuk memastikan teks tidak memecah baris dan menampilkan ... */
        .truncate-text {
            overflow: hidden;
            white-space: nowrap; 
...
<body class="bg-gray-100 <?= trim($body_class) ?>">
                ...
                <div class="bg-white rounded-xl shadow-lg p-4 sm:p-5 lg:p-6">
                    <div class="flex flex-col lg:flex-row justify-between lg:items-center mb-6 gap-4">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-1 mb-2">Kelola pengumuman yang akan ditampilkan di halaman pengumuman.</p>
                        </div>
                        ...
                        <table id="myTable" class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50 sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th class="px-3 py-3 text-left text-xs font-boldkegia text-gray-500 uppercase tracking-wider" style="max-width: 250px;">Judul</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200" id="tableBody">
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-3 py-4 text-sm text-gray-900 font-normal align-middle truncate-text" style="max-width: 250px;">
                                        <?= htmlspecialchars($p['judul']) ?>
                                    </td>
                                    
                                    <td class="px-3 py-4 text-sm text-gray-600 align-middle truncate-text" style="max-width: 350px;">
                                        <?= htmlspecialchars($p['deskripsi']) ?>
                                    </td>
                                    
                                    <td class="px-3 py-4 text-sm text-gray-500 whitespace-nowrap align-middle">
                                        <?= formatTanggal($p['created_at'], 'd M Y')
                                        ?>
                                    </td>
                                    ...
                                    <td class="px-3 py-4 text-sm font-medium text-center whitespace-nowrap align-middle">
                                        <div class="flex items-center justify-center space-x-2">
                                            ...
                                            <button onclick="showDetailModal(<?= htmlspecialchars(json_encode($p)) ?>)" 
                                                    title="Lihat Detail"
                                            ...
                                            <button onclick='editModal(<?= json_encode($p, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) ?>)' 
                                                    title="Edit"
                                            ...
                                            <form method="POST" class="inline" onsubmit="confirmDelete(event, <?= $p['pengumuman_id'] ?>, 'pengumuman')">
    ...
    <div id="modalForm" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-lg p-6 max-w-xl w-full max-h-screen overflow-y-auto">
            <div class="flex justify-between items-center mb-4">
                <h3 id="modalTitle" class="text-xl font-bold">Tambah Pengumuman</h3>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <input type="hidden" name="pengumuman_id" id="pengumumanId">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="judul" id="judul" required maxlength="200"
                               ...
                               placeholder="Judul pengumuman (Maks. 200 karakter)">
                    </div>
                    
                    <input type="hidden" name="kategori" id="kategori" value="N/A">
                    <input type="hidden" name="prioritas" id="prioritas" value="sedang">
                    <input type="hidden" name="status" id="status" value="draft">
                    <input type="hidden" name="tanggal_mulai" id="tanggalMulai" value="<?= date('Y-m-d') ?>">
                    <input type="hidden" name="tanggal_selesai" id="tanggalSelesai" value="">

                    
                    <div>
                        <label class="block font-semibold mb-2">Isi Pengumuman <span class="text-red-500">*</span></label>
                        <textarea name="deskripsi" id="deskripsi" required rows="8" maxlength="2000"
                                 class="w-full px-4 py-2 border rounded-lg"
                                 placeholder="Tulis isi pengumuman di sini (Maks. 2000 karakter)..."></textarea>
                    </div>
    ...
    <div id="detailPengumumanModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-lg p-6 max-w-xl w-full max-h-screen overflow-y-auto">
            <div class="flex justify-between items-center mb-4">
    ...
    <script>
        // --- LOGIKA MODAL EDIT/ADD YANG SUDAH ADA (TIDAK BERUBAH) ---
        function showModal(action) {
            ...
            document.getElementById('kategori').value = 'N/A';
            document.getElementById('prioritas').value = 'sedang';
            document.getElementById('status').value = 'draft';
            document.getElementById('tanggalMulai').value = '<?= date('Y-m-d') ?>';
        ...
        function editModal(data) {
            ...
            document.getElementById('pengumumanId').value = data.pengumuman_id;
            document.getElementById('judul').value = data.judul;
            document.getElementById('deskripsi').value = data.deskripsi;
            
            document.getElementById('kategori').value = data.kategori || 'N/A';
            document.getElementById('prioritas').value = data.prioritas || 'sedang';
            
            document.getElementById('tanggalMulai').value = data.created_at ? data.created_at.substring(0, 10) : '<?= date('Y-m-d') ?>';
            document.getElementById('tanggalSelesai').value = data.tanggal_selesai ? data.tanggal_selesai.substring(0, 10) : '';
        ...
        // --- FUNGSI MODAL DETAIL (DIPERBAIKI) ---

        function showDetailModal(data) {
            // Mengganti karakter newline (\n) dengan tag <br> dan menggunakan break-words
            ...
            const content = `
                <div class="space-y-4">
                    ...
                    <div>
                        <p class="font-semibold text-gray-800">Isi Pengumuman:</p>
                        <p class="mt-1 p-2 bg-gray-50 rounded break-words">${formattedDeskripsi}</p>
                    </div>

                    <div class="flex space-x-4">
                        <div>
                            <p class="font-semibold text-gray-800">Tanggal Dibuat:</p>
                            <p class="mt-1 p-2 bg-gray-50 rounded">${data.created_at.substring(0, 10)}</p>
                        </div>
                    </div>
                </div>
            `;
            document.getElementById('detailContent').innerHTML = content;
            document.getElementById('detailPengumumanModal').classList.remove('hidden');
        }

        function closeDetailModal() {
            document.getElementById('detailPengumumanModal').classList.add('hidden');
        }
        
        // --- LOGIKA MANUAL FILTER, SEARCH, DAN PAGINATION (TIDAK BERUBAH) ---
